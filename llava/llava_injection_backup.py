...
def manually_generate_gpt(input_ids, model, prompt, tokenizer):
    ...
    input_id1 = torch.as_tensor([input_ids]).to(device)
...
def manually_forward(input_ids, model, vision_tower, projector,
        ...
        output_attentions = None,
        output_hidden_states = None,
        return_dict = None, images = None, input_soft_prompt=None):
    
    if images != None and past_key_values == None:
...
def manually_generate(input_ids, model, prompt, tokenizer, vision_tower, projector, images=None, device='cuda:0'):
    temperature = TEMPERATURE
    max_new_tokens = MAX_NEW_TOKENS
    context_len = CONTEXT_LEN
    max_src_len = context_len - max_new_tokens - 8
